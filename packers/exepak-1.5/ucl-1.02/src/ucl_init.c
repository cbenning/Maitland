...
/***********************************************************************
// The next function should get completely optimized out of existance.
************************************************************************/

static ucl_bool basic_check(void)
{
    ucl_bool r = 1;
...
static ucl_bool ptr_check(void)
{
    ucl_bool r = 1;
    int i;
    char _wrkmem[10 * sizeof(ucl_bytep) + sizeof(ucl_align_t)];
    ucl_bytep wrkmem;
    ucl_bytepp dict;
...
#  endif
    ...
    if (r == 1)
    {
        ...
        adler = ucl_adler32(adler, ucl_copyright(), 189);
        r &= __ucl_assert(adler == UCL_UINT32_C(0x56b73963));
    }
