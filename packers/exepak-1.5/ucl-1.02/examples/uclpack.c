...
#define WITH_BENCHMARK 1
   ...
   /* do not introduce floating point dependencies */
#  undef WITH_BENCHMARK
#endif
#if defined(WITH_BENCHMARK)
#include "acc/acclib/uclock.ch"
static acc_uclock_handle_t uc;
...
{
    acc_uclock_t t_start, t_stop;

    acc_uclock_read(&uc, &t_start);
    ...
    acc_uclock_read(&uc, &t_stop);
    benchmark_secs += acc_uclock_get_elapsed(&uc, &t_start, &t_stop);
}
 ...
 */
    ...
    if (opt_test)
    {
        while (i < argc && r == 0)
        {
            ...
            if (r == 0 && benchmark_bytes > 0)
            {
                ...
                double mbs = 0;
                if (benchmark_secs >= 0.0001)
                    mbs = mb / benchmark_secs;
                printf("\n  [benchmark] %ld loops, %.3f MB, %.3f secs, %.3f MB/s\n\n",
    ...
    {
        printf("%s: using block-size of %ld bytes\n", progname, (long)opt_block_size);
